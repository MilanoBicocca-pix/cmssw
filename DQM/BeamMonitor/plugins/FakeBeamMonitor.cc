...
void FakeBeamMonitor::beginLuminosityBlock(const LuminosityBlock& lumiSeg, const EventSetup& context) {
  ...
  if (StartAverage_) {
    ...
    if (debug_)
      edm::LogInfo("FakeBeamMonitor") << " beginLuminosityBlock:  Size of mapBeginBSLS before =  " << mapBeginBSLS.size()
                                  << endl;
    if (nthlumi >
...
void FakeBeamMonitor::endLuminosityBlock(const LuminosityBlock& lumiSeg, const EventSetup& iSetup) {
  ...
  edm::LogInfo("FakeBeamMonitor") << "endLuminosityBlock:: Lumi of the last event before endLuminosityBlock: " << nthlumi
                              << endl;
...
//  theBeamFitter->SetPVInfo(DipPVInfo_);
  ...
  if (onlineMode_) {  // filling LS gap
    ...
    if (currentlumi % fitPVNLumi_ == 0)
      LSgap_pv--;
    edm::LogInfo("FakeBeamMonitor") << "FitAndFill::  LSgap_bs = " << LSgap_bs << " ; LSgap_pv = " << LSgap_pv << std::endl;
