#ifndef BeamSpot_BeamSpotExt_h
#define BeamSpot_BeamSpotExt_h
/** \class reco::BeamSpotExt
 *  
...
namespace reco {
  ...
  public:
    ...
    BeamSpotExt( const Point &point,
		         ...
		         const CovarianceMatrix &errorPV,
		         double xPV,
		         double yPV,
		         double dxdzPV,
		         double dydzPV,
    ...
    /// x coordinate from PV fit
    double xPV() const { return xPV_; }
    /// y coordinate from PV fit 
    double yPV() const { return yPV_; }
    ...
    /// dxdy slope 
    double dxdy() const { return dxdy_; }
    /// beam width X
    double BeamWidthX() const { return BeamWidthX_; }
	/// beam width Y
    double BeamWidthY() const { return BeamWidthY_; }
    /// dxdz slope from PV fit
    double dxdzPV() const { return dxdzPV_; }
    /// dydz slope from PV fit 
    double dydzPV() const { return dydzPV_; }
    ...
    /// error on dxdy
    double dxdyError() const { return sqrt ( error_(8,8) ); }

    /// error on beam width X, assume error in X = Y
    double BeamWidthXError() const { return sqrt ( error_(6,6) );}
	/// error on beam width Y, assume error in X = Y
    double BeamWidthYError() const { return sqrt ( error_(7,7) );}
		
    /// error on quantities from PV fit
    double xPVError()    const { return sqrt( errorPV_(0,0) ); }
    double yPVError()    const { return sqrt( errorPV_(1,1) ); }
    double dxdzPVError() const { return sqrt( errorPV_(4,4) ); }
    double dydzPVError() const { return sqrt( errorPV_(5,5) ); }
    ...
    double covariancePV( int i, int j) const {
      return errorPV_(i,j);
    }
    /// return full covariance matrix of dim 9
    CovarianceMatrix covariance()   const { return error_; }
    CovarianceMatrix covariancePV() const { return errorPV_; }
  ...
  ///
  std::ostream& operator<< ( std::ostream&, BeamSpotExt beam );
