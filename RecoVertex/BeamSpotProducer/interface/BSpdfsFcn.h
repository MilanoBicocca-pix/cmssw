...
class BSpdfsFcn : public ROOT::Minuit2::FCNBase 
{
  public:
    ...
    virtual double Up         		(void						   ) const {return 1.;  	     }
	
  private:
    double         PDFGauss_d           (double 				 z     ,
                                         double 		    		 d     ,
					 double 				 sigmad,
    				         double phi, const std::vector<double> & parms     ) const;
    double         PDFGauss_d_resolution(double 				 z     ,
                                         double 				 d     ,
    					 double  				 phi   ,
					 double 				 pt    ,
					 const std::vector<double>             & parms     ) const;

    double         PDFGauss_z           (double z, 
                                         double sigmaz, 
					 const std::vector<double>             & parms     ) const;
