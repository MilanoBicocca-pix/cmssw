...
reco::BeamSpot BSFitter::Fit(double *inipar = 0) 
{
  ...
  else if ( ffit_variable == "d*z" || ffit_variable == "default" ) 
  {
    if ( ffit_type == "likelihood" || ffit_type == "default" ) 
    {
      ...
      reco::BeamSpot spot(reco::BeamSpot::Point(
                                                tmp_d0phi.x0()        ,
                                                tmp_d0phi.y0()        ,
						tmp_d0phi.z0())       ,
  	    				      	tmp_d0phi.sigmaZ()    ,
      ...
      if (ffit_type == "likelihood") 
      {
        ...
        {
         edm::LogWarning("BSFitter") << "BSFitter: Result is non physical. Log-Likelihood fit to extract beam width did not converge."
        			     << std::endl;
    ...
    else if ( ffit_type == "resolution" ) 
    {
      ...
      if (edm::isNotFinite(ff_minimum)) 
      {
        edm::LogWarning("BSFitter") << "Result is non physical. Log-Likelihood fit did not converge." 
        		<< std::endl;
        tmp_lh.setType(reco::BeamSpot::Unknown);
        return tmp_lh;
      }
...
reco::BeamSpot BSFitter::Fit_z_likelihood(double *inipar) 
{
  ...
  */
  ...
  return reco::BeamSpot( reco::BeamSpot::Point(
                                               0.			 ,
  					       0.			 ,
...
reco::BeamSpot BSFitter::Fit_z_chi2(double *inipar) 
{
  ...
  //delete h1z; //!!!!! Memory leak!!!!!

  return reco::BeamSpot( 
                        reco::BeamSpot::Point(0.      ,
  			        	      0.      ,
  			        	      fpar[0]),
  			fpar[1],
  			0.     ,
...
reco::BeamSpot BSFitter::Fit_ited0phi() 
{
  ...
  {
    edm::LogWarning("BSFitter") << "need at least " 
                                << fminNtrks 
				<< " tracks to run beamline fitter." 
				<< std::endl;
  ...
  edm::LogInfo("BSFitter") << "Total number of successful iterations = " 
                           << ( goodfit ? (fnthite+1) : fnthite ) 
			   << std::endl;
  if (goodfit) 
  {
...
reco::BeamSpot BSFitter::Fit_d0phi() 
{
  ...
  if (fnthite > 0) edm::LogInfo("BSFitter") << " number of tracks used: " 
                                            << ftmprow 
					    << std::endl;
  ...
  {
    goodfit = false;
    edm::LogWarning("BSFitter") << "Decomposition failed, matrix singular ?" 
                                << std::endl
                                << "condition number = " 
				<< bk.Condition() 
				<< std::endl;
