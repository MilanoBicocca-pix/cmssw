...
{
  _HERE_() ;
  edm::ParameterSet _ps = iConfig.getParameter<edm::ParameterSet>("BeamFitter") ;
  debug_                = _ps.getUntrackedParameter<bool>("Debug");
  
  tracksToken_       	= iColl.consumes<reco::TrackCollection>   
                     	  (
		     	   _ps.getUntrackedParameter<edm::InputTag>("TrackCollection")
		     	  );
  ...
  saveBeamFit_       	= _ps.getUntrackedParameter<bool>		      ("SaveFitResults"        );
  savePVVertices_    	= _ps.getUntrackedParameter<bool>		      ("SavePVVertices"        );
  isMuon_            	= _ps.getUntrackedParameter<bool>		      ("IsMuonCollection"      );

  trk_MinpT_         	= _ps.getUntrackedParameter<double>		      ("MinimumPt"	       );
  trk_MaxEta_        	= _ps.getUntrackedParameter<double>		      ("MaximumEta"	       );
  trk_MaxIP_         	= _ps.getUntrackedParameter<double>		      ("MaximumImpactParameter");
  trk_MaxZ_          	= _ps.getUntrackedParameter<double>		      ("MaximumZ"	       );
  trk_MinNTotLayers_ 	= _ps.getUntrackedParameter<int>		      ("MinimumTotalLayers"    );
  trk_MinNPixLayers_ 	= _ps.getUntrackedParameter<int>		      ("MinimumPixelLayers"    );
  trk_MaxNormChi2_   	= _ps.getUntrackedParameter<double>		      ("MaximumNormChi2"       );
  trk_Algorithm_     	= _ps.getUntrackedParameter<std::vector<std::string> >("TrackAlgorithm"        );
  trk_Quality_       	= _ps.getUntrackedParameter<std::vector<std::string> >("TrackQuality"	       );
  min_Ntrks_         	= _ps.getUntrackedParameter<int>		      ("MinimumInputTracks"    );
  convergence_       	= _ps.getUntrackedParameter<double>		      ("FractionOfFittedTrks"  );
  inputBeamWidth_    	= _ps.getUntrackedParameter<double>		      ("InputBeamWidth",-1.    );
  ...
  {
    outputfilename_ = _ps.getUntrackedParameter<std::string>("OutputFileName");
...
void BeamFitter::readEvent(const edm::Event& iEvent)
{
	...
	if (fnormchi2 < trk_MaxNormChi2_)         { countPass[3] += 1;
	  if (falgo)                              { countPass[4] += 1;
		...
		if (std::abs( fz0 ) < trk_MaxZ_)  { countPass[7] += 1;
		  if (fpt > trk_MinpT_)           { countPass[8] += 1;
		    if (std::abs( feta ) < trk_MaxEta_ /* && fpvValid */) 
		    {
		      if (debug_) 
		      {
                        edm::LogInfo("BeamFitter") << "Selected track quality = " 
			                           << track->qualityMask()
                                                   << "; track algorithm = " 
						   << track->algoName() 
						   << "= TrackAlgorithm: " 
  ...
  for( std::vector<BSTrkParameters>::const_iterator iparam  = fBSvector.begin(); 
                                                    iparam != fBSvector.end(); 
						  ++iparam) 
  {
...
{
  edm::LogInfo("BeamFitter") << " [BeamFitterDebugTime] freftime[0] = " 
                             << freftime[0]
			     << "; address =  " 
			     << &freftime[0]
			     << " = " 
			     << fbeginTimeOfFit 
			     << std::endl;
  edm::LogInfo("BeamFitter") << " [BeamFitterDebugTime] freftime[1] = " 
                             << freftime[1]
			     << "; address =  " 
			     << &freftime[1]
			     << " = " 
  ...
  if(fBSvector.size() > 1 )
  {
    edm::LogInfo("BeamFitter") << "Calculating beam spot..." 
                               << std::endl
    			       << "We will use " 
			       << fBSvector.size() 
			       << " good tracks out of " 
    ...
    delete myalgo;
    if ( fbeamspot.type() != 0 ) 
    {// save all results except for Fake (all 0.)
      fit_ok = true;
      if (saveBeamFit_)
      {
	fx 	   = fbeamspot.x0();
  ...
  { // tracks <= 1
    ...
    fbeamspot = tmpbs;
    fbeamspot.setType(reco::BeamSpot::Fake);
    edm::LogInfo("BeamFitter") << "Not enough good tracks selected! No beam fit!"
                               << std::endl;
...
bool BeamFitter::runBeamWidthFitter() 
{
  ...
  {
    fbeamspot.setType(reco::BeamSpot::Fake);
    edm::LogWarning("BeamFitter") << "Not enough good tracks selected! No beam fit!" 
                                  << std::endl;
  }
  return widthfit_ok;
}
...
void BeamFitter::dumpTxtFile(std::string & fileName, bool append)
{
  ...
  if ( MyPVFitter->IsFitPerBunchCrossing() ) 
  {
    for (std::map<int,reco::BeamSpot>::const_iterator abspot  = fbspotPVMap.begin(); 
                                                      abspot != fbspotPVMap.end(); 
						    ++abspot) 
    {
...
void BeamFitter::write2DB()
{
  ...
  pBSObjects->SetPosition(
                          fbeamspot.position().X(),
			  fbeamspot.position().Y(),
			  fbeamspot.position().Z()
			 );
